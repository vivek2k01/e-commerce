...
const AdminFeaturedProducts = () => {
  ...
  useEffect(() => {
    const itemsRef = ref(db, "FeaturedProducts");
  ...
  const handleSave = () => {
    ...
    set(ref(db, `FeaturedProducts/${formData.id}`), formData)
      .then(() => {
  ...
  const handleDelete = (id) => {
    remove(ref(db, `FeaturedProducts/${id}`))
  ...
  return (
    ...
    >
      <Typography>
        <h1 style={{ fontFamily: "Georgia, serif" }}>Featured Products</h1>
      </Typography>
